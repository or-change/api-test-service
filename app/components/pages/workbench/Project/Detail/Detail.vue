	...
	<div class="mt-3">
		...
		<div class="mt-3">
			...
			<div>
				...
				<b-button size="sm" variant="danger" @click="deleteSource" class="float-right">删除</b-button>
				<b-pagination
			...
			>
				...
				<template slot="[semver]" slot-scope="data">
					<b-link :disabled="data.item.status !== 5" :to="`/workbench/project/${projectId}/source/${data.item.id}`">{{ data.value }}</b-link>
				</template>
...
export default {
	...
	methods: {
		selectAll(checked) {
			...
			return this.selectedSourceList = this.$refs.sourceList.filteredItems.map(source => source.id);
		},
