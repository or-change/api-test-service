...
function normalize(options) {
	function normalizeOptionsNode(optionsNode) {
		...
		const {
			name: _name = finalOptionsNode.name,
		...
		if (typeof _name !== 'string') {
			throw new Error('Invalid `options.name`, string expected.');
		}
...
module.exports = function KoaRouterPluginProvider(originalOptions) {
	...
	return function KoaRouterPlugin(app, injection) {
